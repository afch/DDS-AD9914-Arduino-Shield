...
void AD9914::initialize(unsigned long refClk, uint8_t N){
  ...
  byte registerInfo11[] = { CFR1, 4 };
  byte data11[] = { 0, ENABLE_SINE_OUTPUT, 0, EXTERNAL_POWER_DOWN_CONTROL | SDIO_INPUT_ONLY };  // Grisha when PLL Enabled VCO_CAL_ENABLE
  //byte data11[] = { 0, ENABLE_SINE_OUTPUT, 0, SDIO_INPUT_ONLY };  // Grisha when PLL Enabled VCO_CAL_ENABLE
  AD9914::writeRegister(registerInfo11, data11);
  AD9914::update();

  AD9914::disableSyncClck();
  /*
      byte registerInfo[] = {CFR1, 4};
      byte data[] = {VCO_CAL_ENABLE, ENABLE_SINE_OUTPUT, 0, EXTERNAL_POWER_DOWN_CONTROL | SDIO_INPUT_ONLY}; // Grisha when PLL Enabled VCO_CAL_ENABLE 
      AD9914::writeRegister(registerInfo, data);
      AD9914::update();
      */

  //**** tyt bil santehnik
  byte registerInfo2[] = { CFR2, 4 };
  byte data2[] = { 0x00, 0x00, 0x09, 0x00 };  //
  ...
  if (N) {
    ...
    byte data3[] = { 0x00,  // bilo 0x00
                    PLL_enable,
                    N,                                   //32,  // N Value PLLMultiplier, PLL_OUT = IN_REF_CLK x 2 x N
                    Manual_ICP_selection | ICP_750uA | Lock_detect_enable};  // //[31:0] //bilo 0 // Manual_ICP_selection | ICP_750uA
    AD9914::writeRegister(registerInfo3, data3);

    byte registerInfo[] = { CFR1, 4 };
    byte data[] = { 0x01, 0x80, 0x01, EXTERNAL_POWER_DOWN_CONTROL | SDIO_INPUT_ONLY };  //{VCO_CAL_ENABLE, ENABLE_SINE_OUTPUT, 0, EXTERNAL_POWER_DOWN_CONTROL | SDIO_INPUT_ONLY}; // Grisha when PLL Enabled VCO_CAL_ENABLE
    AD9914::writeRegister(registerInfo, data);
    AD9914::update();
  } else 
  {
...
// byte data[] = {0x00, ENABLE_SINE_OUTPUT, OSK_ENABLE, EXTERNAL_POWER_DOWN_CONTROL | SDIO_INPUT_ONLY}; // Alec when PLL Disabled
  ...
  //byte data[] = {VCO_CAL_ENABLE, ENABLE_SINE_OUTPUT, OSK_ENABLE, SDIO_INPUT_ONLY}; // Grisha when PLL Enabled VCO_CAL_ENABLE 
  AD9914::writeRegister(registerInfo, data);
  AD9914::update();
...
// Return 32 bits
uint32_t AD9914::readRegister4Bytes(byte registerAddress)
{
  uint32_t data = 0;
  digitalWrite(_ssPin, LOW);

  SPI.transfer(registerAddress | B10000000); // Modify address for reading
...
void AD9914::dacCalibrate(){
  ...
  byte data[] = {DAC_CAL_ENABLE, 0x05, 0x21, 0x20}; //write bit high {DAC_CAL_ENABLE, 0x05, 0x21, 0x20};
  AD9914::writeRegister(registerInfo, data);
  AD9914::update();
  delay(1); // bilo 10
 ...
 AD9914::writeRegister(registerInfo, data); 
 AD9914::update();
  //  data[0] = Aux_divider_power_down; //clear DAC_CAL_ENABLE and disable Aux_divider_power_down
  // AD9914::writeRegister(registerInfo, data);
  // AD9914::update();
